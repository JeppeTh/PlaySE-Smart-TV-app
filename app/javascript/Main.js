...
Main.onLoad = function(refresh) {
    ...
    if (!this.loaded) {
        ...
        tizenVersion = tizen.systeminfo.getCapabilities().platformVersion;
        tizenVersion = +tizenVersion.replace('.','');
        if (deviceYear > 2011)
            LINE_LENGTH = 36;
        Log('Model:' + model +  ' DeviceYear:' + deviceYear + ' IsEmulator:' + isEmulator + ' application:' + tizen.application.getCurrentApplication().appInfo.name + ' Cookies:' + document.cookie + ' version:' + webapis.productinfo.getSmartTVServerVersion() + ' firmware:' + webapis.productinfo.getFirmware() + ' tizen version:' + tizenVersion);
        loadingStart();
        Main.setClock();
        checkDateFormat();
        Footer.display();
